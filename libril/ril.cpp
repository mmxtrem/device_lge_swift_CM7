...
#define LOG_TAG "RILC_miroslav_mm"
...
dispatchInts (Parcel &p, RequestInfo *pRI) {
	...
	/* libril-qc apparently only supports SERVICE_NONE here */
        if (pRI->pCI->requestNumber == RIL_REQUEST_QUERY_CALL_WAITING)
            pInts[i] = 0;
        else if (pRI->pCI->requestNumber == RIL_REQUEST_SET_CALL_WAITING && i == 1)
            pInts[i] = 0;
        else
            pInts[i] = (int)t;
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
    ...
    if (pRI->pCI->requestNumber == RIL_REQUEST_BASEBAND_VERSION) {
        ...
        property_get("ro.ril.baseband", baseband, "QCT unknown");
        response=strdup(baseband);
    }

    /* The PLMN strings on libril-qc are idiotic, replace
     * them with the SPN */
    if (responselen && pRI->pCI->requestNumber == RIL_REQUEST_OPERATOR) {
        char **p_cur = (char **) response;
        if (p_cur[1] != NULL && strlen(p_cur[1])) {
            p_cur[0] = strdup(p_cur[1]);
        }
    }
